...
(function () {
  ...
  //   3. We stamp a discreet Momentum-Light footer into the bottom-right
  //      corner so the PNG reads as a branded export.
  //   4. The resulting canvas is downloaded as `momentum-timeline-<iso>.png`.
  //
  // UX: a floating toast reports progress ("capture en cours…", "rendu de
  // la Timeline…", "export prêt ✓") so the user knows the click registered
  // — html2canvas on a wide plan can take a couple of seconds.
  // ---------------------------------------------------------------------------
    ...
    async function runExport() {
      ...
      try {
        ...
        const canvas = await window.html2canvas(root, {
          ...
          allowTaint: false,
          logging: false,
          // Scale governs the output resolution. Cap at 2× so wide plans
          // don't generate 20 MB PNGs, but still match Retina clarity.
          scale: Math.min(window.devicePixelRatio || 1, 2),
          // Match the viewport Jira renders against. Without these,
          // html2canvas sometimes clips the chart to the default 800×600.
          windowWidth: document.documentElement.clientWidth,
          windowHeight: document.documentElement.clientHeight,
          ignoreElements: isCaptureChrome,
          // Forward scroll position so the exported image matches the
          // user's current view of the timeline.
