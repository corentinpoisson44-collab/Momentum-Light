...
| 11 | **Classement par date de début** — chip `↕ Classer par date de début` dans le bandeau Momentum de la Timeline. Au clic, réordonne les Epics visibles en écrivant le champ de classement JIRA (`PUT /rest/agile/1.0/issue/rank`, chaîne `rankAfterIssue`) comme un drag & drop manuel. La date de début utilisée est la position de la barre sur l'axe du chart (gauche → début) — aucun custom field à découvrir, on classe sur ce que l'utilisateur voit. L'ordre précédent est sauvegardé dans `localStorage` (`momentum-light::timeline-sort-snapshot`) et le bouton bascule en `↩ Restaurer l'ordre des Epics` (teinte bleue) ; un second clic réapplique l'ordre snapshot via la même API et efface le snapshot. Toast de progression `n/N` pendant les requêtes séquentielles ; échec API surfacé en toast. | ✅ v0.11.0 |
...
- `timelineSort` — classement à la volée des Epics visibles par leur date de début (gauche de la barre sur l'axe). `collectEpics()` filtre les barres via `issueMeta.isEpic`, `applyOrder()` chaîne des `rankAfterIssue` séquentiels sur `/rest/agile/1.0/issue/rank`, et un snapshot localStorage (`momentum-light::timeline-sort-snapshot`) permet le revert depuis le même bouton
