...
| 7 | **Projection Timeline** — bouton `📅 Projection` à côté du bandeau vélocité. Ouvre une modale qui séquence en FIFO tous les Epics de la Timeline (ordre vertical = ordre de priorité) à partir de leur macro-chiffrage T-Shirt (`max(macroSP, chiffrage réel) − done`) et de la vélocité moyenne. Deux scénarios sont calculés en parallèle : **probable** (capacité = vélocité moyenne) et **prudent** (capacité = vélocité − 1σ). Chaque Epic reçoit un sprint d'atterrissage + une date de fin. Au-delà des sprints actifs/futurs connus, les sprints sont extrapolés `Sprint +N` via la durée moyenne observée. Le sprint en cours est compté à pleine capacité (optimiste, explicité dans la modale). | ✅ v0.8.0 |
