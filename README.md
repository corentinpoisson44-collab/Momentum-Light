...
| 7 | **Projection Timeline** — bouton `📅 Projection` à côté du bandeau vélocité + badge inline `→ Sprint N · date` à l'extrémité droite de chaque barre d'Epic T-shirté. Séquence en FIFO tous les Epics de la Timeline (ordre vertical = ordre de priorité) à partir de leur macro-chiffrage T-Shirt (`max(macroSP, chiffrage réel) − done`) et de la vélocité moyenne. Deux scénarios sont calculés en parallèle : **probable** (capacité = vélocité moyenne) et **prudent** (capacité = vélocité − 1σ). Chaque Epic reçoit un sprint d'atterrissage + une date de fin. Au-delà des sprints actifs/futurs connus, les sprints sont extrapolés `Sprint +N` (badge en hachuré violet) via la durée moyenne observée. Le sprint en cours est compté à pleine capacité (optimiste, explicité dans la modale). Le badge inline affiche le scénario probable ; le tooltip révèle le scénario prudent. | ✅ v0.9.0 |
...
- `velocityBanner` — bandeau fixe sur les vues timeline/plan + bouton `Projection`
- `timelineProjection` — séquence FIFO des Epics T-shirtés dans la vélocité moyenne (et `vélocité − 1σ` pour le scénario prudent), mappe sur les sprints actifs/futurs connus puis extrapole via la durée moyenne. Expose une modale avec un tableau par Epic (sprint probable + sprint prudent) et la date de livraison globale
